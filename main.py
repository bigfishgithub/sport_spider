...
from api.video_stream import video_stream_fetch
...
from jobs.video_stream_job import VideoStreamJob
...
async def run_producer_and_consumer(task_config):
	...
	for config in task_config:
		...
		process_funcs = {config['identifier']: config['process_func']}

		# 创建多个消费者
		for _ in range(config.get('num_consumers', 50)):
			consumer = Consumer(process_funcs, processing_done_event, config['identifier'])
...
async def main():
	task_config = [
		# 任务配置
		{'func': coach_list_fetch, 'interval': 1, 'identifier': 'coach_list', 'process_func': CoachJob.run},
		{'func': competition_list_fetch, 'interval': 1, 'identifier': 'competition_list','process_func': CompetitionJob.run},
		{'func': competition_rule_list_fetch, 'interval': 1, 'identifier': 'competition_rule_list','process_func': CompetitionRuleListJob.run},
		{'func': delete_data_fetch, 'interval': 1, 'identifier': 'delete_data', 'process_func': DeleteJob.run},
		{'func': honor_list_fetch, 'interval': 1, 'identifier': 'honor_list', 'process_func': HonorListJob.run},
		{'func': live_stream_fetch, 'interval': 1, 'identifier': 'live_stream', 'process_func': LiveStreamJob.run},
		...
		{'func': match_live_update_fetch, 'interval': 1, 'identifier': 'match_live_update', 'process_func': MatchLiveJob.run},
		{'func': more_update_fetch, 'interval': 1, 'identifier': 'more_update', 'process_func': MoreUpdateJob.run},
		{'func': player_list_fetch, 'interval': 1, 'identifier': 'player_list', 'process_func': PlayerListJob.run},
		...
		{'func': referee_list_fetch, 'interval': 1, 'identifier': 'referee_list', 'process_func': RefereeListJob.run},
		{'func': season_list_fetch, 'interval': 1, 'identifier': 'season_list', 'process_func': SeasonListJob.run},
		{'func': stage_list_fetch, 'interval': 1, 'identifier': 'stage_list', 'process_func': StageListJob.run},
		{'func': table_detail_fetch, 'interval': 1, 'identifier': 'table_detail', 'process_func': TableDetailJob.run},
		...
		{'func': venue_list_fetch, 'interval': 1, 'identifier': 'venue_list', 'process_func': VenueListJob.run},
		{'func': video_stream_fetch, 'interval': 1, 'identifier': 'video_stream', 'process_func': VideoStreamJob.run},
	]
